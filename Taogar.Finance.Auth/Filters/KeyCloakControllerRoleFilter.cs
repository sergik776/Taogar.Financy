...
{
    public class KeyCloakControlerRoleFilter : IAuthorizationFilter
    {
        private string[] Roles { get; set; }

        public KeyCloakControlerRoleFilter()
        {

        }

        public KeyCloakControlerRoleFilter(string roles)
        {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            logger.Info<KeyCloakControlerRoleFilter>("проверка роли пользователя", ConsoleColor.DarkYellow);
            if (!UserHasPermission(context.HttpContext.User, config))
            {
                logger.Info<KeyCloakControlerRoleFilter>("доступ к контроллеру запрещен", ConsoleColor.DarkYellow);
                throw new BaseHTTPError(403, "У вас недостаточно прав доступа к этой сущности или операции");
            }
            else
            {
                logger.Info<KeyCloakControlerRoleFilter>("доступ к контроллеру разрешен", ConsoleColor.DarkYellow);
            }
        }
